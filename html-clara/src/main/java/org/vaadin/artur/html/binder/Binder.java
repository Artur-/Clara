...
import org.vaadin.artur.html.binder.annotation.UiField;
import org.vaadin.artur.html.binder.annotation.UiHandler;
...
public class Binder {
	...
	public void bind(Component componentRoot, Object controller,
			Map<String, Component> globalIds, Map<String, Component> localIds,
			Map<String, Component> captions) {
		if (controller == null) {
			return;
		}

		bindFields(componentRoot, controller, controller.getClass());
		bindMethods(componentRoot, controller, globalIds, localIds, captions);
	}

	/**
	 * Returns a {@link Map} of all controller fields decorated with the
	 * {@link UiField} annotation that already have a {@link Component}
	 * reference assigned. If the given controller is {@code null}, an empty
	...
	private void bindMethods(Component componentRoot, Object controller,
		...
		for (Method method : methods) {
			if (method.isAnnotationPresent(UiDataSource.class)) {
				bindDataSource(componentRoot, controller, method, globalIds,
						localIds, captions);
			}
	...
	private void bindDataSource(Component componentRoot, Object controller,
			Method method, Map<String, Component> globalIds,
			Map<String, Component> localIds, Map<String, Component> captions) {
		String componentId = method.getAnnotation(UiDataSource.class).value();
		Component component = localIds.get(componentId);
		if (component == null)
			component = globalIds.get(componentId);
		if (component == null)
			component = captions.get(componentId);

		// Component component = tryToFindComponentById(componentRoot,
		// componentId);
